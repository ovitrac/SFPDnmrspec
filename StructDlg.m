...
%     'factory defaults'.
%
%    See also: Struct2str, Browse_Struct, StructDlgPrepare
%
...
% Revision INRA\Olivier Vitrac - 14/10/2014 Severak fixes, comptability with StructDlgPrepare
...
if (exist('dflt','var') ~= 1) || (isempty(dflt) && ~isstruct(dflt))
    dflt = struct([]);
end
...
if (isstruct(struct_def)) % Init
    ...
    [struct_def,units,limits,protected,struct_length] = split_def(struct_def);
    ...
    h_fig = figure( ...
        ...
        'WindowStyle',         'modal'); % Change to normal when debugging (instead of modal)
    ...
    ud.col   = col;
    ud.width = max(structfun(@length,struct_length)); %,20;
    ud.units     = units;
    ud.dflt      = dflt;
    ud.limits    = limits;
    ud.protected = protected;
    ud.length    =  struct_length;
...
%%
%%%%%%%%%%%%%
function [struct_def,units,limits,protected,struct_length] = split_def(struct_def)
units     = struct([]);
limits    = struct([]);
protected = struct([]);
fnames = fieldnames(struct_def);
...
% length extraction (INRA\Olivier Vitrac)
lengthextaction = false;
if isstruct(struct_def) && nargout>4
    if all(structfun(@iscell,struct_def)) && all(structfun(@length,struct_def)>4)
        struct_length = structfun(@(p) p{5},struct_def,'UniformOutput',false);
        struct_maxlength = max(cellfun(@(f) struct_length.(f),fnames(~structfun(@isempty,struct_length))));
        for f = fnames(structfun(@(x) isempty(x) || isnan(x),struct_length))'
            struct_length.(f{1}) = struct_maxlength;
        end
        lengthextaction = true;
    end
end
if (nargout>4) && ~lengthextaction, struct_length = cell2struct(repmat({NaN},nfvals,1),fnames); end
% end length extraction
...
for i = 1:length(f_units)
    if (ischar(v_units{i}) && ~isempty(v_units{i}))
        index = strcmpi(f_units{i},fnames);
        if (~isempty(index))
...
if (isempty(h))
    ...
    % length update by INRA\Olivier Vitrac
    if ~isempty(ud.length.(f)) && ~isnan(ud.length.(f))
        udlength = ud.length.(f);
    else
        if ischar(val), udlength = length(val)+10; else udlength = 20; end
    end
    % end length update
    ...
    h = uicontrol(h_fig, ...
        'style',      'edit', ...
        'Units',      'char', ...
        'position',   [lbl_pos(3)+lbl_pos(1)+0.5 lbl_pos(2) udlength 1.5],... 20
        'string',     [], ...
        'FontName',   'Helvetica', ...
        'FontSize',   9, ...
    ...
    if (ischar(val))
        h1 = uicontrol(h_fig, ...
            'style',            'text', ...
            'units',            'char', ...
            'position',         [lbl_pos(3)+lbl_pos(1)+0.5+21.5 lbl_pos(2) udlength 1.5],... length(val)+10
            'String',           ['=(''' val ''')'], ...
            'FontName',         'Helvetica', ...
            'fontsize',         9, ...
...
set(h,'String',val);
pos = get(h,'Position');
pos(3) = max(udlength,length(val)+10); %max(20,length(val)+10);
set(h,'Position',pos);
    ...
    case 'pushbutton'
        ...
        switch (push_ud.cmd)
            case {'uigetfile','uiputfile','uigetdir'}
                ...
                % fix INRA\Olivier Vitrac - 12/10/2014
                extra_args = regexprep(extra_args,'^.*\}','');
                % end after
...
% ################################################################################"
% Function previously included in the toolbox but inserted now as private ones
...
switch (class(x))
    ...
    case {'double' 'sparse'}
        if (isempty(x))
            xstr = '[]';
        elseif (~ismatrix(x) || min(size(x)) >1 || length(x) >150)
            dims = size(x);
    ...
    case 'cell'
        ...
        if (isempty(x))
            xstr = '{}';
        elseif (~ismatrix(x) || min(size(x)) >1)
            dims = size(x);
