function varargout = struct2dlg(varargin)
%  STRUCTDLG Struct dialog box.
%
%    ANSWER = STRUCT2DLG(S) creates a modal dialog box that returns user
...
% 
%       sout = struct2dlg(s,'Title',options);
%
%
%    Example 2:
%
...
%
    ...
    function SubStructOpenFcn(hObject,eventdata,fname)%#ok
        if fcnoutput
            sdata.(fname) = struct2dlg(sdata.(fname),iIdentifier2String(fname),options);
        else
            struct2dlg(sdata.(fname),iIdentifier2String(fname),options);
        end
    end
