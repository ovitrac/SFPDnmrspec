...
% Revision INRA\Olivier Vitrac
%   19/01/2014 new search engine to find ghostscript (it is not anymore shipped with Matlab since R2013b)
   ...
   if ispc
      if exist(fullfile(ghostDir,'bin','win32','gs.exe'), 'file')
         gsCmd = fullfile(ghostDir,'bin','win32','gs.exe');
          ...
          gsCmd = gs_path;
          ghostDir = rootdir(rootdir(gs_path));
      end
   else 
