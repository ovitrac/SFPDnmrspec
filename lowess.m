...
% additional variables (INRA\Olivier Vitrac tunning)
dispon = false;
...
if exist('wantplot','var') == 0 || wantplot == 0
    ...
    if dispon, fprintf('No plot will be created.\n'); end %#ok<*UNRCH>
    wantplot = 0;
    imagefile = '';
    limits = 1;
    upperLimit = nan;
...
if exist('imagefile','var') == 0
    ...
    if dispon, fprintf('No plot will be exported.\n'); end
    imagefile = '';
end

% Revision OV 01/08/10
f_default = .25;
if nargin<2, f = []; end
if isempty(f), f = f_default; end
if numel(f)~=size(datain,1), f = f(ones(size(datain,1),1)); end
f(f<=0) = f_default; f(f>1) = 1;
% end revision
...
for nplots=1:limits
    ...
    for j=1:n
        m=fix(n*f0(j)+0.5); % modified by OV 01/08/10
...
fprintf('\tLOWESS\tCompute time %6.4f seconds.\n',toc(start));
...
if xdata
    ...
    if ~isnan(xyd)
        ...
        if length(xd) ~= length(xdata)
            fprintf('One or more x-values were beyond the range supplied to lowess.\nOr there were duplicate values.\nThey will be ignored.\n');
        end
    else
        if dispon, fprintf('There were NaNs in the lowess results. No plot will be created.\n'); end
        wantplot = 0;
        xy = nan;
    end
end
...
if wantplot ~= 0
    customplot(dataout,upperLimit,lowerLimit,f0,[dte x_data y_data],xy,imagefile);
end
...
if dd == 0.0
    ...
    while dd <= ddmax
        % unused variable, removed by OV 01/08/10
        %c = 0;
        %total = 0.0;
        f = (abs(x-xx)/dd);
        f = 1.0-f.^3;
...
try
    ...
    xlabel('x-Values')
    ylabel('y-Values')
    ts = strcat('LOWESS Regression plot f=',num2str(mean(f)));
    title(ts)
    ...
    if ~isempty(imgfile)
        ...
        fprintf('Creating plot. Give a few tics.\n');
