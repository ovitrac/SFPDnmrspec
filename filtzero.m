function xf = filtzero(x,m)
% FILTZERO zero phase filtering (based on filtfilt)

% MS 23/04/04 INRA\Olivier Vitrac rev. 16/09/12

% 05/02/08 add a copy of filtfilt to work without the signal analysis toolbox
% 16/09/12 fix m<=1

% arg check
if nargin<2, m = ceil((size(x,1))/100); end
if m<2, xf=x; return, end
...
%FILTFILT Zero-phase forward and reverse digital filtering (copy of FILTFILT in the signal analysis toolbox).
